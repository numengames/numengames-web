...
const SITE = {
	kicker: "NUMEN GAMES · THE COMPANY",
	name: "Numen Games",
	line: "We design participatory experiences: narrative, game dynamics, live facilitation.",
	domain: "numen.games",
};
...
const FONT_FILES = [
	"geist/files/geist-latin-wght-normal.woff2",
	"geist-mono/files/geist-mono-latin-wght-normal.woff2",
];
const KHEPRI = path.join(root, "src", "assets", "brand", "Khepri_Logo.svg");
...
function check() {
	...
	if (w !== W || h !== H) {
		console.error(
			`share-card --check: og-card.png is ${w}×${h}, must be ${W}×${H}`,
		);
		process.exit(1);
	}
