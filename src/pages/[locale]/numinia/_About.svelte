			...
			<div class="flex gap-2 flex-col mt-8 md:mt-14 relative text-left">
				...
				<div>
					<PulseAnimatedBtn
						...
						className="bg-primary-coralRed mt-4 md:mt-6"
						link={textObj?.ctaButtons?.primary?.link}
						text={textObj?.ctaButtons?.primary?.text} />
				</div>
				<div
					class="absolute -top-[45px] md:-top-[65px] left-0 text-6xl md:text-9xl text-transparent stroke-red-500 stroke-1 font-IBMPlexMono"
	...
	<div class="flex justify-between items-center w-full mt-8 md:mt-10 overflow-x-auto pb-4">
		{#each textObj?.steps as step, index}
			<button
				...
				<div
					class="flex items-center justify-center w-6 h-6 md:w-8 md:h-8 rounded-full bg-primary-coralRed text-white font-IBMPlexMono text-sm md:text-base">
					{step?.number}
				</div>
