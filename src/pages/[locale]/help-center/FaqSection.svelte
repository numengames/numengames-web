    ...
    <div class="flex-1">
      {#each list as section, index (section.title)}
        <div id={`section-${index}`} class="px-4 lg:px-0 scroll-mt-[120px]">
          ...
          <div
            class={`flex flex-col items-center lg:items-end mt-6 ${
              index !== list.length - 1 ? "mb-10" : ""
            }`}
          >
    ...
    >
      ...
      <div class="mt-4 flex flex-col gap-2">
        {#each list as section, index (section.title)}
          <button
            type="button"
            class={`block w-full text-left px-0 py-1 bg-transparent border-none m-0 cursor-pointer transition-colors duration-200 focus:outline-none focus-visible:ring-1 focus-visible:ring-primary-coralRed/40 ${
              activeSection === index
                ? "text-primary-coralRed"
                : "text-primary-beige/15 hover:text-primary-beige/60"
            }`}
