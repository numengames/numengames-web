...
<div class="w-full flex flex-col relative">
	<div class="w-full flex flex-col items-start z-10">
		...
		<div class="w-full flex justify-between items-center">
			<h2 class="text-4xl 2xl:text-5xl font-normal mt-3 mb-6 text-primary-beige pr-10">
		...
		{#each textObj?.content as yearContent, yearIndex (yearIndex)}
			{#each Object.entries(yearContent) as [year, texts] (year)}
				<button class="flex h-full cursor-pointer relative mb-8" on:click={() => toggleYearSelection(year)} on:keydown={(e) => e.key === "Enter" && toggleYearSelection(year)}>
					<h3 class={`year-title font-geistMono text-xl md:text-2xl ${selectedYear === year ? "text-ambar" : "text-primary-beige/40"}`}>
						{year}
					</h3>
					<div class="h-full px-8 md:px-16 flex flex-col items-center">
						<div class="flex w-full items-center justify-center relative z-10">
							<div class={`rounded-full top-0 ${selectedYear === year ? "bg-primary-coralRed" : "bg-[#8A7D72]/50"} w-6 h-6 opacity-40 absolute`}></div>
							<div class={`rounded-full top-1.5 ${selectedYear === year ? "bg-primary-coralRed" : "bg-[#8A7D72]"} w-3 h-3 absolute`}></div>
						</div>
						{#if yearContent !== textObj.content[textObj.content.length - 1]}
							<div class="absolute w-0.5 h-full top-2.5 bg-primary-panther"></div>
						{/if}
					</div>
					<div class="flex flex-col h-full flex-1">
						{#each texts as text, index (index)}
							<p class={`text-left text-base md:text-lg leading-relaxed ${index === texts.length - 1 ? "pb-8 md:pb-16" : "pb-4"} ${selectedYear === year ? "text-primary-beige/75" : "text-primary-beige/40"}`}>
								{text}
							</p>
						{/each}
					</div>
