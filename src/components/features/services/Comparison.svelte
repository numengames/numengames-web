...
<div class="w-full flex flex-col relative px-4 md:px-8 lg:px-0">
	<div class="w-full flex flex-col items-start z-10">
		...
		<div class="w-full flex flex-col md:flex-row justify-between items-start md:items-center gap-4 md:gap-0">
			<h2 class="text-2xl md:text-3xl lg:text-4xl 2xl:text-5xl font-normal mt-3 mb-4 md:mb-6 text-primary-beige md:pr-10">
	...
	<div class="w-full flex flex-col md:flex-row gap-4 md:gap-6 mt-6">
		{#each Object.entries(textObj?.list) as [key, texts] (key)}
			<div class={`w-full rounded-xl py-6 md:py-8 lg:py-10 px-4 md:px-6 ${key === "before" ? "border border-arena/5" : "border border-[#EFA51740] border-opacity-25 shadow-[0_0_50px_0_rgba(239,165,23,0.10)] bg-[#1E1A17]"}`}>
				<p class={`mb-6 md:mb-8 text-lg md:text-xl ${key === "before" ? "text-primary-beige" : "text-primary-coralRed"}`}>
					{key.charAt(0).toUpperCase() + key.slice(1)}
				</p>
				{#each texts as text, index (index)}
					<div class={`text-primary-beige/75 flex items-start md:items-center py-4 md:py-6 ${index !== texts.length - 1 ? "mb-3 md:mb-4" : ""} border rounded-lg ${key === "before" ? "border-arena/5 bg-[linear-gradient(90deg,rgba(20,17,15,0.70)_24%,rgba(20,17,15,0.00)_100%)]" : "border-primary-coralRed/25"}`}>
						<div class={`flex justify-center items-center rounded-full ml-4 md:ml-5 w-5 h-5 md:w-6 md:h-6 mt-1 md:mt-0 ${key === "before" ? "bg-arena/10" : "bg-primary-coralRed"}`}>
							{#if key === "before"}
